# - Selectively use some of the Find-modules provided by cxx11-cmake-modules (CXX11CM)
#
# CXX11CM_USE_FIND_MODULES(DIR <dir> MODULES module1.cmake module2.cmake .. moduleN.cmake [NO_OVERRIDE] )
#
# This macro is automatically available once cxx11-cmake-modules has been found, so it is not
# necessary to include(CXX11CMUseFindModules) explictely.
#
# Use this macro instead of simply adding ${CXX11CM_FIND_MODULE_DIR} to CMAKE_MODULE_PATH
# if you want to use only a subset or some specific find-modules coming with cxx11-cmake-modules.
...
#   MODULES   : the list of find-modules to be used from cxx11-cmake-modules
#   NO_OVERRIDE: if used, the given modules will only be used if they do not exist in the currently used
#                CMake version
#
# Example:
#   find_package(CXX11CM 0.0.1 REQUIRED)
#   cxx11cm_use_find_modules(DIR ${CMAKE_BINARY_DIR}/cmake/ Modules FindBlueZ.cmake NO_OVERRIDE )
...
function(cxx11cm_use_find_modules)
   ...
   if(NOT EUFM_DIR)
      message(FATAL_ERROR "No DIR specified for cxx11cm_use_find_modules() !")
   endif()
   ...
   foreach(file ${EUFM_MODULES})
      if(NOT EXISTS ${CXX11CM_FIND_MODULE_DIR}/${file} )
         message(FATAL_ERROR "File ${file} not found in ${CXX11CM_FIND_MODULE_DIR} !")
      endif()
      if(NOT EXISTS "${CMAKE_ROOT}/Modules/${file}" OR NOT EUFM_NO_OVERRIDE)
          configure_file("${CXX11CM_FIND_MODULE_DIR}/${file}" "${EUFM_DIR}/${file}" COPYONLY)
      endif()
   endforeach()
